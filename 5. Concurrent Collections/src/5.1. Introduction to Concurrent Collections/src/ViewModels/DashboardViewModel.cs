...
public sealed partial class DashboardViewModel : BaseViewModel, IAsyncDisposable
{
	...
	readonly Dictionary<string, StockQuoteModel> _latestStockQuotes = [];
	...
	async Task UpdateStockPrice(string symbol, CancellationToken token)
	{
		try
		{
			...
			if (!_latestStockQuotes.TryAdd(symbol, quote))
			{
				_latestStockQuotes[symbol] = quote;
			}
	...
	{
		List<StockSymbolModel> stockSymbolList = [];

		foreach (var (symbol, companyName) in _stockSymbols)
		{
